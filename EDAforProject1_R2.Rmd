...
IBUmaxState <- mergeDF %>% filter(!is.na(IBU)) %>% group_by(State) %>% dplyr::summarise(IBUmax=max(IBU))
ABVmaxState <- mergeDF %>% filter(!is.na(ABV)) %>% group_by(State) %>% dplyr::summarise(ABVmax=max(ABV))
...
mergeDF_noNA$IPA_Ale <- ifelse(str_detect(mergeDF_noNA$Style,regex("IPA", ignore_case = TRUE)),"IPA",ifelse(str_detect(mergeDF_noNA$Style,regex("Ale",ignore_case = TRUE)),"Ale","Other"))

# Visualization with ABV data
##   Bloxplot
mergeDF_noNA %>% ggplot(aes(y=ABV, x=IPA_Ale, fill=as.factor(Ounces))) + geom_boxplot() +
  xlab("Ale Types (IPA or ALE)") + ylab("ABV") + ggtitle("Boxplot of ABV vs Ale Type")
mergeDF_noNA %>% ggplot(aes(y=ABV, x=as.factor(Ounces), fill=IPA_Ale)) + geom_boxplot() +
  xlab("Ounces") + ylab("ABV") + ggtitle("Boxplot of ABV vs Ounces")
##   Histogram
mergeDF_noNA %>% ggplot(aes(x=ABV)) + geom_histogram(fill="blue", color="black")
##   Barchart
mergeDF_noNA %>% ggplot(aes(x=ABV, fill=IPA_Ale)) + geom_bar(color="black") + xlim(c(0.025,0.100)) +
  xlab("Alcohol By Volume (ABV)") + ylab("Beer Name Count") + ggtitle("Ale/IPA Weight vs ABV")
mergeDF_noNA %>% ggplot(aes(x=ABV, fill=IPA_Ale)) + geom_bar(color="black", position = "fill") + xlim(c(0.025,0.100)) +
  xlab("Alcohol By Volume (ABV)") + ylab("Beer Name Count%") + ggtitle("Ale/IPA Weight% vs ABV")
mergeDF_noNA %>% ggplot(aes(x=ABV, fill=as.factor(Ounces))) + geom_bar(color="black") + xlim(c(0.025,0.100)) +
  xlab("Alcohol By Volume (ABV)") + ylab("Beer Name Count") + ggtitle("Ounce Weight vs ABV")

# Visualization with IBU data
##   Bloxplot
mergeDF_noNA %>% ggplot(aes(y=IBU, x=IPA_Ale, fill=as.factor(Ounces))) + geom_boxplot() +
  xlab("Ale Types (IPA or ALE)") + ylab("IBU") + ggtitle("Boxplot of IBU vs Ale Type")
mergeDF_noNA %>% ggplot(aes(y=IBU, x=as.factor(Ounces), fill=IPA_Ale)) + geom_boxplot() +
  xlab("Ounces") + ylab("IBU") + ggtitle("Boxplot of IBU vs Ounces")
##   Barchart
mergeDF_noNA %>% ggplot(aes(x=IBU, fill=IPA_Ale)) + geom_bar(color="black") +
  xlab("International Bitterness Units (IBU)") + ylab("Beer Name Count") + ggtitle("Ale/IPA Weight vs IBU")
mergeDF_noNA %>% ggplot(aes(x=IBU, fill=IPA_Ale)) + geom_bar(color="black", position = "fill") +
  xlab("International Bitterness Units (IBU)") + ylab("Beer Name Count%") + ggtitle("Ale/IPA Weight% vs IBU")
mergeDF_noNA %>% ggplot(aes(x=IBU, fill=as.factor(Ounces))) + geom_bar(color="black") +
  xlab("International Bitterness Units (IBU)") + ylab("Beer Name Count") + ggtitle("Ounce Weight vs IBU")
# Scatter plot ABV vs. IBU
mergeDF_noNA %>% ggplot(aes(x=IBU, y=ABV)) + geom_point(shape=0) + ggtitle("ABV vs IBU") + geom_smooth(method = lm)
mergeDF_noNA %>% ggplot(aes(x=IBU, y=ABV, color=IPA_Ale)) + geom_point() + ggtitle("ABV vs IBU") + geom_smooth(method = lm)
mergeDF_noNA %>% ggplot(aes(x=IBU, y=ABV, color=IPA_Ale)) + geom_point() + ggtitle("ABV vs IBU") + facet_wrap(~State)
mergeDF_noNA %>% ggplot(aes(x=IBU, y=ABV, color=IPA_Ale)) + geom_point() + ggtitle("ABV vs IBU") + facet_wrap(~as.factor(Ounces))
```
...
ggplotly()
cor.test(mergeDF$IBU, mergeDF$ABV)
mod <- lm(mergeDF$IBU ~mergeDF$ABV)
summary(mod)

# Scatter plot and Pearson's correlation with tidy data (no missing value)
mergeDF_noNA %>% ggplot(aes(x=ABV, y=IBU)) + 
  geom_point(aes(color=State), size=1, position="jitter") + 
  geom_smooth(method = lm) + 
  ggtitle("Relationship between IBU and ABV")
ggplotly()
cor.test(mergeDF_noNA$IBU, mergeDF_noNA$ABV)
mod1 <- lm(mergeDF_noNA$IBU ~ mergeDF_noNA$ABV)
summary(mod1)
```
...
```{r}
table(mergeDF_noNA$IPA_Ale) # Ale:963, IPA:571, Other:876
# Check the Ale: 2nd time with regex(): # Ale:1007, IPA:571, Other:832

mergeDF_noNA.knn <- mergeDF_noNA %>% filter(IPA_Ale == "IPA" | IPA_Ale == "Ale")
...
for(j in 1:iterations)
{
  ...
  trainIndices = sample(1:dim(mergeDF_noNA.knn)[1],round(splitPerc * dim(mergeDF_noNA.knn)[1]))
  train = mergeDF_noNA.knn[trainIndices,]
  test = mergeDF_noNA.knn[-trainIndices,]
  for(i in 1:numks)
    {
...
```{r}
mergeDF_noNA$BudweiserOrOther <- ifelse(str_detect(mergeDF_noNA$Brewery_name,regex("Budweiser", ignore_case = TRUE)),"Budweiser","Other")
table(mergeDF_noNA$BudweiserOrOther)
head(mergeDF_noNA %>% filter(BudweiserOrOther=="Budweiser"),n=50)
```
